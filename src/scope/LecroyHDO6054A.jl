...
## MEASUREMENTS
# Set Measurement
function set_meas(scope::T,ch=1,fct=0,par=1,vw=1) where {T<:LCR6xxx}
...
## TRIGGER
# Set trigger source
set_trig_src(scope::T,ch=1) where {T<:LCR6xxx} = write(scope.obj,"VBS app.Acquisition.Trigger.Edge.Source=$ch")
# Set trigger type
set_trig_typ(scope::T,tp=0) where {T<:LCR6xxx} = write(scope.obj,"VBS app.Acquisition.Trigger.Type=$tp")
# Set trigger mode
set_trig_mode(scope::T,mode=0) where {T<:LCR6xxx} = write(scope.obj,"VBS app.Acquisition.TriggerMode=$mode")
# Set trigger slope
set_trig_slope(scope::T,slp=0) where {T<:LCR6xxx} = write(scope.obj,"VBS app.Acquisition.Trigger.Edge.Slope=$slp")
# Set trigger level
...
set_trig_cpl(scope::T,ch=1,cpl=0) where {T<:LCR6xxx} = write(scope.obj,"VBS app.Acquisition.Trigger.C$(ch)Coupling=$cpl")
