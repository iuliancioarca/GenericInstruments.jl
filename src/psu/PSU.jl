...
## OUTPUT FUNCTION
# Set voltage range
set_range(psu::T;ch::Int=1,vrang="low") where {T<:E364x1} = write(psu.obj,"VOLT:RANG $vrang")
set_range(psu::T;ch::Int=1,vrang="low") where {T<:E364x2} = write(psu.obj,"INST output$ch;VOLT:RANG $vrang")
# Set output voltage
set_volt(psu::T;ch::Int=1,volt=0) where {T<:E364x1} = write(psu.obj,"VOLT $volt")
set_volt(psu::T;ch::Int=1,volt=0) where {T<:E364x2} = write(psu.obj,"INST output$ch;VOLT $volt")
# Set compliance (current)
set_compl(psu::T;ch::Int=1,crtlim=0) where {T<:E364x1} = write(psu.obj,"CURR $crtlim")
set_compl(psu::T;ch::Int=1,crtlim=0) where {T<:E364x2} = write(psu.obj,"INST output$ch;CURR $crtlim")

## OUTPUT
# Set output on/off
set_outp(psu::T;ch::Int=1,st="off") where {T<:E364x1} = write(psu.obj,"OUTP $st")
set_outp(psu::T;ch::Int=1,st="off") where {T<:E364x2} = write(psu.obj,"INST output$ch;OUTP $st")

# high level example with multiple parameters
#function set_volt_compl(psu::T;ch::Int=1,volt=0,crtlim=0.1) where {T<:Union{E364x1,E364x2}}
